...
这个 Cloudflare Worker 接收 Apple `notarytool --webhook` 通知，使用 Durable Object 按 stable tag 与源 GitHub Actions run 聚合 `aarch64`、`x64` 两个回调。只有两个架构都进入 Apple 终态后，relay 才向 `JimmyDaddy/CoreRobin-Internal` 发送一次 `apple-notarization-complete` repository dispatch。
...
Release workflow 会为 Apple 自动追加 `tag`、`run_id` 和 `arch` query 参数。
...
- Relay 以 tag/run ID 命名 Durable Object，串行聚合两个架构，并用 `dispatching`/`dispatched` 状态抑制并发或重复回调。
