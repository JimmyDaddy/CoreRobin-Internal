...
if (options.dryRun) {
  console.log(JSON.stringify({
    ...
    runTimeoutMilliseconds: options.runTimeout,
  }, null, 2));
  process.exit(0);
}
...
for (let run = 1; run <= options.runs; run += 1) {
  ...
  if (result.error?.code === "ETIMEDOUT") {
    throw new Error(
      `Monitor benchmark run ${run} exceeded ${options.runTimeout} ms. ` +
      "Check for a stale or unresponsive mounted volume before retrying.",
    );
  }
  if (result.status !== 0) {
...
function parseOptions(args) {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    else if (["--runs", "--iterations", "--spacing", "--run-timeout", "--output"].includes(argument)) {
      const value = args[index + 1];
      if (value === undefined) throw new Error(`Missing value for ${argument}.`);
      const key = argument === "--run-timeout" ? "runTimeout" : argument.slice(2);
      parsed[key] = key === "output" ? value : Number(value);
      index += 1;
    } else throw new Error(`Unknown option: ${argument}`);
  }
  for (const key of ["runs", "iterations", "spacing", "runTimeout"]) {
