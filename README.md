...
连接自己的 Ollama、本机或云端模型服务，向 Robin 询问设备状态并逐步查看实际检查结果。发送前可选择设备上下文并预览内容；结束进程或移到废纸篓前仍需确认。能力中心还提供可直接使用的清理、进程、网络和历史工具，本地输入与操作结果不会自动发送给模型。

详见[AI 助手使用指南](docs/user-guide.zh-CN.md#ai-助手)与[模型服务隐私说明](docs/privacy.zh-CN.md#ai-助手与模型服务)。
...
- 应用级 1 小时/24 小时/7 天影响归因、系统事件回放、电池使用会话、个性化本机基线与真实登录启动对比
- 系统盘快速/完整扫描、外置卷与指定文件夹完整扫描；结果写入有界的分层目录索引，进入目录无需重扫整盘，未缓存的目录可按需载入并支持后台刷新
- 清理后区分选择的逻辑大小、实际处理的磁盘占用与文件系统可用空间变化
- 按责任应用归组的现代后台项、Bundle ID、Team ID、签名、真实可执行文件与孤儿项提示
- 应用关注规则、历史事件、桌面提醒与恢复通知
- 关闭主窗口后仍运行的原生采样器，以及主窗口、状态栏面板和 Robin 小窗口共享的后台状态
...
清理操作始终先加入清理篮，再明确选择“移到废纸篓”或“直接删除”。执行时会逐项安全绑定：仍存在的普通对象按选择处理，不存在的直接跳过，无法访问或不支持的对象单独报告，不会阻塞其他已确认项目。点号应用目录、具体应用支持文件和偏好数据可以在“必须确认”状态下处理；凭据、密钥、邮件、信息、钥匙串、照片图库、云同步根、系统根、链接、特殊对象和其他用户数据仍受保护。用户明确选择的文件夹或外置卷只获得边界内、同文件系统、当前用户对象的处理权限。
...
- 监控、历史和偏好默认保存在当前设备；主动使用 AI 时，所选内容会发送给配置的模型服务
- 默认不把应用名称或连接地址写入历史；连接历史需单独启用，并只保存本机五分钟应用/主机聚合
- 用户确认执行的退出、重启、清理、卸载、卷推出、启动项和更新操作会记录结果，并提供失败重试与操作后趋势回执；清理记录不保存路径
- 不会自动结束进程、卸载应用或删除文件
...
当前 macOS 安装包已使用 Developer ID 签名、经过 Apple 公证并装订票据，发布前还会通过 Gatekeeper 校验。Windows 安装包尚未配置平台签名，Windows 与 Linux 仍是早期预览。Release 同时提供 SHA-256 校验表、SPDX SBOM，以及校验表的 Sigstore 签名包；这些来源完整性记录不能替代其他平台签名或真实设备验收。
