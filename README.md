...
进程操作不会只依赖 PID。采样快照会尽可能记录操作系统提供的高精度进程启动标识；加载详情时在刷新前后校验该标识，确认框只使用同一次已核验详情，执行信号前还会再次复核。任何一次标识不一致都会拒绝执行，从而降低 PID 被复用后误操作新进程的风险。
...
macOS 的最终信号接口仍然基于 PID，因此当前属于多重校验的 best-effort 防护，而不是稳定进程句柄提供的原子保证。Linux `pidfd` 与 Windows 同一进程 handle 上的校验和终止，是下一阶段的平台安全工作。
