...
- 提供面向普通用户的一键智能诊断，用人话解释电脑是否正常、为什么可能变慢
- 检查持续高 CPU、内存压力、低磁盘空间、持续高磁盘 I/O 和异常大的网络活动
- 将同一应用的辅助进程合并归因，并只引导用户查看证据，不自动结束进程或删除文件
...
- 作为独立页面完整扫描下载、废纸篓、应用缓存、开发缓存和用户隐藏目录，不设置静默截断结果的时间或数量上限
- 扫描在后台运行，持续显示当前路径、已检查项目、已发现空间和耗时，并允许用户主动停止
- 使用可下钻的扇形空间图展示分类、扫描根目录和主要子目录，面积按实际文件大小计算；点击进入目录，拖动扇区到清理篮
- 扫描结果会在本机保留 7 天，超过 24 小时会提示更新；只有进入某个目录时才低频核对该层变化，不做全盘实时监听
- macOS 会只读盘点系统能够确认已超过 6 个月未打开的应用；未知使用时间不会被误判，也不会提供自动卸载
- 清理篮区分“通常可回收”和“必须确认”；复核路径、扫描后变化和一次性确认后才移入系统废纸篓，不做永久删除
- 只读取文件元数据、不读取文件内容、不跟随符号链接；权限不足的位置会单独列出
...
- 默认不保存应用显示名；用户可单独允许在有明确归因证据时保存应用名，且仍不保存进程命令、用户、路径、文件名或连接地址
- 所有历史只留在当前设备，不会上传或同步
...
- 桌面提醒带持续判定、恢复回差、冷却和每日上限，可按 CPU、内存、磁盘类别分别关闭
...
- `src-tauri/src/cleanup.rs`：文件扫描、应用盘点、进度/取消、空间层级聚合与受保护的系统废纸篓操作
