...
export function DailyGuide(props: DailyGuideProps) {
  ...
  return (
    <section className={`daily-guide daily-guide--${props.intent}`} aria-labelledby="daily-guide-title">
      ...
      {!refreshing && !refreshedResult && props.intent === "slow" ? <SlowGuide {...props} /> : null}
      {!refreshing && !refreshedResult && props.intent === "space" ? <SpaceGuide {...props} /> : null}
      {!refreshing && props.intent === "startup" ? <StartupGuide {...props} /> : null}
      {!refreshing && !refreshedResult && props.intent === "heat" ? <HeatGuide {...props} /> : null}
      {!refreshing && !refreshedResult && props.intent === "network" ? <NetworkGuide {...props} /> : null}
      {!refreshing && !refreshedResult && props.intent === "checkup" ? <CheckupGuide {...props} /> : null}
    </section>
  );
}
...
function HeatGuide({ diagnosis, snapshot, preparingAction, onOpenApplications, onRequestClose }: DailyGuideProps) {
  ...
  return (
    <section className={`daily-guide-result is-${tone}`}>
      ...
      <div className="daily-guide-result__content">
        <span className="eyebrow">{t("daily.guide.result")}</span>
        <h2>{t(`daily.guide.heat.result.${cause}.title`, { name: sleep?.blocker.name })}</h2>
...
function CheckupGuide({ diagnosis, snapshot, onOpenIntent }: DailyGuideProps) {
  const { t } = useTranslation();
  const attentionItems = buildDailyAttentionItems(diagnosis, snapshot);
  ...
  const presentation = attentionPresentation(primary, t);
  return (
    <section className={`daily-guide-result is-${primary.level}`}>
      <span className="daily-guide-result__icon"><TriangleAlert size={23} /></span>
      ...
      <div className="daily-guide-result__actions"><button className="button button--primary" type="button" onClick={() => onOpenIntent(primary.intent)}>{t("daily.guide.checkup.openPriority")}<ArrowRight size={14} /></button></div>
    </section>
  );
}

function attentionPresentation(
  item: ReturnType<typeof buildDailyAttentionItems>[number],
  t: TFunction,
) {
  ...
  if (item.kind === "application") {
    return {
      title: t("daily.attention.application.title", { name: item.application.name }),
      description: t(`daily.attention.application.${item.impact}`),
    };
  }
  if (item.kind === "sleep") {
    return {
...
function RecheckCard({
  ...
  const primary = intent === "checkup"
    ? buildDailyAttentionItems(diagnosis, snapshot)[0] ?? null
    : null;
  return (
      ...
      <div className="daily-recheck__content">
        ...
        <div className="daily-recheck__actions">
          ...
          {intent === "checkup" && primary ? (
            <button className="button button--primary" type="button" onClick={() => onOpenIntent(primary.intent)}>{t("daily.guide.checkup.openPriority")}<ArrowRight size={14} /></button>
          ) : null}
          {intent === "space" ? (
            <button className="button button--secondary" type="button" onClick={onOpenSpace}>{t("daily.guide.space.viewSpace")}<ArrowRight size={14} /></button>
...
function recheckPresentation(
  ...
  if (intent === "checkup") {
    const primary = buildDailyAttentionItems(diagnosis, snapshot)[0] ?? null;
    if (primary) {
      const presentation = attentionPresentation(primary, t);
      return {
        tone: primary.level,
