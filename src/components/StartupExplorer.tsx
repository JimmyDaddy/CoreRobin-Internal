...
export function StartupExplorer({
  ...
  const visible = useMemo(
    () => filterStartupItems(items, filter, query).slice(0, 100),
    [filter, items, query],
  );
  ...
  return (
    <section className="startup-explorer" aria-labelledby="startup-title">
      ...
      {!snapshot && loading ? (
        ...
        <>
          <section className="startup-summary" aria-label={t("startup.summary")}>
...
function StartupItemRow({
  ...
  return (
    <article className={`startup-item is-${advice}`}>
