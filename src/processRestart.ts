...
export async function waitForProcessReplacement(
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      const snapshot = await sample();
      if (snapshot.processes.some((process) =>
        processIdentity(process) !== previousIdentity &&
        process.name.trim().toLocaleLowerCase() === normalizedName)) {
        return true;
      }
    } catch {
      return false;
