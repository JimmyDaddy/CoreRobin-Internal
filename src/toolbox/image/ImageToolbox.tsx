import { CircleAlert, Download, FileImage, Play, RotateCcw, ShieldCheck, Square, Upload } from "lucide-react";
...
import { C2PA_READER_MAX_BYTES, inspectEmbeddedC2pa } from "./c2paReader";
...
import { createBrowserImageInputs, createNativeImageInputs, strictImageMimeType, type ImageRunInputs } from "./imageInputs";
...
export function ImageToolbox({ toolId, deliverOutput: externalDeliverOutput }: { toolId: ImageToolId; deliverOutput?: ImageOutputDelivery }) {
  ...
  const selectManifest = (selected: File | undefined) => {
    if (!selected) return;
    if (selected.size > C2PA_READER_MAX_BYTES) {
  ...
  const runManifest = () => void (async () => {
    ...
    try {
      ...
      if (desktopRuntime) {
        ...
        const bytes = await readBoundToolboxInput(nativeInputJob, token, controller.signal, C2PA_READER_MAX_BYTES);
        const buffer = bytes.buffer.slice(bytes.byteOffset, bytes.byteOffset + bytes.byteLength) as ArrayBuffer;
        file = new File([buffer], token.displayName, { type: strictImageMimeType(bytes) });
      }
      if (!file) throw new Error(t("image.noManifest"));
      if (file.size > C2PA_READER_MAX_BYTES) throw new Error(t("image.manifestTooLarge"));
      const bytes = new Uint8Array(await file.slice(0, 16).arrayBuffer());
      const format = strictImageMimeType(bytes);
  ...
  return <div className="toolbox-tool-layout image-toolbox">
    <div className="toolbox-tool-layout__body">
      ...
      {toolId === "c2pa-inspector" ? <div className="toolbox-inline-actions">{!desktopRuntime ? <label className="toolbox-file-pick button button--secondary"><Upload size={14} />{t("image.selectManifest")}<input hidden type="file" accept="image/png,image/jpeg,image/webp" disabled={running} onChange={(event) => selectManifest(event.target.files?.[0])} /></label> : <span className="toolbox-hint"><FileImage size={14} />{t("image.nativeManifestHint")}</span>}<button className="button button--primary" type="button" disabled={running || Boolean(nativeOutput) || Boolean(nativeReportOutput) || (!desktopRuntime && !manifestFile)} onClick={runManifest}><Play size={14} />{t("image.inspectManifest")}</button>{running ? <button className="button button--secondary" type="button" disabled={stopping} onClick={() => void stop()}><Square size={14} />{stopping ? t("image.stopping") : t("image.stop")}</button> : null}<button className="button button--secondary" type="button" disabled={running} onClick={() => { void cancelNativeOutput(); void cancelNativeReport(); setManifestFile(null); setManifestReport(""); clearOperationReport(); if (manifestDownloadUrlRef.current) URL.revokeObjectURL(manifestDownloadUrlRef.current); manifestDownloadUrlRef.current = ""; setManifestDownloadUrl(""); setNotice(""); setError(""); }}><RotateCcw size={14} />{t("image.clear")}</button></div> : toolId === "image-recipe" || toolId === "image-editor" ? <div className="toolbox-inline-actions">{running ? <button className="button button--secondary" type="button" disabled={stopping} onClick={() => void stop()}><Square size={14} />{stopping ? t("image.stopping") : t("image.stop")}</button> : null}<button className="button button--secondary" type="button" disabled={running} onClick={() => { void cancelNativeOutput(); void cancelNativeReport(); setFiles([]); releaseOutputs(); setRecipientDelivery(null); setNotice(""); setError(""); }}><RotateCcw size={14} />{t("image.clear")}</button></div> : <div className="toolbox-inline-actions"><button className="button button--primary" type="button" disabled={running || Boolean(nativeOutput) || Boolean(nativeReportOutput) || !hostExecutorAvailable || (!desktopRuntime && files.length === 0)} onClick={action}><Play size={14} />{stopping ? t("image.stopping") : running ? t("image.processing") : actionLabel(toolId, t)}</button>{running ? <button className="button button--secondary" type="button" disabled={stopping} onClick={() => void stop()}><Square size={14} />{stopping ? t("image.stopping") : t("image.stop")}</button> : null}<button className="button button--secondary" type="button" disabled={running} onClick={() => { void cancelNativeOutput(); void cancelNativeReport(); setFiles([]); setLogoFile(null); setFontFile(null); releaseOutputs(); setRecipientDelivery(null); setNotice(""); setError(""); }}><RotateCcw size={14} />{t("image.clear")}</button></div>}
      {running ? <div className="image-toolbox__progress" role="status"><progress max="1" value={progress} aria-label={t("image.processing")} /></div> : null}
      {error ? <p className="toolbox-error image-toolbox__error" role="alert"><CircleAlert size={16} aria-hidden="true" />{error}</p> : null}
      {notice ? <pre className="toolbox-notice image-toolbox__notice" role="status">{notice}</pre> : null}
      {manifestReport ? <div className="toolbox-output image-toolbox__output"><pre className="toolbox-notice">{manifestReport}</pre>{manifestDownloadUrl && !desktopRuntime ? <a className="button button--secondary" download="corerobin-c2pa-manifest-report.json" href={manifestDownloadUrl}><Download size={14} />{t("image.downloadReport")}</a> : null}</div> : null}
      {robustnessReport ? <div className="toolbox-output image-toolbox__output"><pre className="toolbox-notice">{robustnessReport}</pre></div> : null}
      {operationReport ? <div className="toolbox-output image-toolbox__output"><pre className="toolbox-notice">{operationReport}</pre>{desktopRuntime ? nativeReportOutput?.outputToken ? <div className="toolbox-inline-actions"><button className="button button--secondary" type="button" onClick={() => void saveNativeReport()}><Download size={14} />{t("image.saveFormal")}</button><button className="button button--secondary" type="button" onClick={() => void cancelNativeReport()}>{t("image.cancelTemporary")}</button></div> : <button className="button button--secondary" type="button" onClick={() => void prepareNativeReport()}><Download size={14} />{t("image.saveFormal")}</button> : operationReportUrl ? <a className="button button--secondary" download={operationReportName} href={operationReportUrl}><Download size={14} />{t("image.downloadReport")}</a> : null}</div> : null}
      {toolId === "recipient-tracking" && recipientDelivery ? <p className={`${recipientDelivery.status === "ready" ? "toolbox-hint" : "toolbox-error"} image-toolbox__delivery-status`} role={recipientDelivery.status === "ready" ? "status" : "alert"}>{t("image.recipientStatusLabel")}: {t(`image.recipientStatus.${recipientDelivery.status}`)} · {recipientDelivery.delivered}/{recipientDelivery.requested} · {recipientDelivery.detail}</p> : null}
      {result ? <div className="image-toolbox__result" role="region" aria-label={t("toolLayout.result")}>
        <div className="image-toolbox__result-header"><strong>{t("toolLayout.result")}</strong><span>{resultLabel(result)}</span></div>
        <div className="image-toolbox__preview"><img src={result.uri} alt={t("image.resultAlt")} /></div>
        <div className="toolbox-inline-actions image-toolbox__result-actions">{nativeOutput?.outputToken ? <><button className="button button--secondary" type="button" onClick={() => void saveNativeOutput()}><Download size={14} />{t("image.saveFormal")}</button><button className="button button--secondary" type="button" onClick={() => void cancelNativeOutput()}>{t("image.cancelTemporary")}</button></> : null}{deliverFormalOutput ? <button className="button button--secondary" type="button" onClick={() => void deliverFormalOutput(markerResultOutput(result)).then(() => setNotice(t("image.handedToProvider")), (reason: unknown) => setError(reason instanceof Error ? reason.message : t("image.formalDeliveryFailed")))}><Download size={14} />{t("image.deliverFormal")}</button> : null}{!desktopRuntime ? <a className="button button--secondary" download={result.filename ?? "corerobin-watermarked.png"} href={result.uri}><Download size={14} />{t("image.downloadPreview")}</a> : null}<span className="toolbox-hint">{nativeOutput?.outputToken ? t("image.nativeOutputSummary", { sizeKiB: Math.ceil(nativeOutput.outputToken.byteLength / 1024) }) : null}</span></div>
      </div> : null}
      {!result && nativeOutput?.outputToken ? <div className="toolbox-inline-actions image-toolbox__prepared-output"><button className="button button--secondary" type="button" onClick={() => void saveNativeOutput()}><Download size={14} />{t("image.saveFormal")}</button><button className="button button--secondary" type="button" onClick={() => void cancelNativeOutput()}>{t("image.cancelTemporary")}</button><span className="toolbox-hint">{t("image.nativeOutputSummary", { sizeKiB: Math.ceil(nativeOutput.outputToken.byteLength / 1024) })}</span></div> : null}
      {zipUrl && !desktopRuntime ? <a className="button button--primary image-toolbox__zip-download" download="corerobin-watermarks.zip" href={zipUrl}><Download size={14} />{t("image.downloadZip")}</a> : null}
    </div>
