...
2026-09-01 的工具箱生命周期、C2PA sidecar、桌面字体 token、补丁计划归档与唤醒状态接线后，
最终生产产物为 3,003,905 bytes JS 与 436,655 bytes CSS；按用户已授权的总量调整，当前预算为
3,100,000 / 450,000 bytes，仍保留约 3% / 3% 余量。本次实机范围只要求 macOS，Windows/Linux
不做实机验证但保留跨平台实现；图片/补丁运行时内存和输出预算没有放宽。
