...
2026-09-01 的工具箱生命周期、C2PA 本地只读解析、桌面字体 token、补丁计划归档与唤醒状态接线后，
普通 production JS/CSS 产物为 3,414,023 / 437,453 bytes；按用户已授权的总量调整，当前普通资源
预算为 3,450,000 / 450,000 bytes。C2PA 的懒加载入口独立计量为 33,936 bytes JS 与 8,269,371
bytes WASM，预算为 75,000 / 10,000,000 bytes；它不计入普通 JS/CSS 总量，且不进入首屏入口。
本次实机范围只要求 macOS，Windows/Linux 不做实机验证但保留跨平台实现；图片/补丁运行时内存和
输出预算没有放宽。
