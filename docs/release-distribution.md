...
2. verify/build job 以只读权限测试源码并构建各平台安装包；build job 仅通过 `TAURI_SIGNING_PRIVATE_KEY` 与 `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` 生成 Tauri 更新签名，不向其他 job 暴露私钥。
...
Tauri 更新签名用于防止应用内更新包被替换，不能代替 Developer ID、Apple 公证或 Windows Authenticode 等平台签名。后者仍决定操作系统是否把首次安装识别为受信发布者。
...
将令牌保存为 `CoreRobin-Internal` 仓库的 Actions secret `PUBLIC_RELEASE_TOKEN`。将 Tauri 更新私钥和密码分别保存为 `TAURI_SIGNING_PRIVATE_KEY` 与 `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`；仓库只保留可公开的更新公钥。不要把令牌或更新私钥写进源码、workflow 文件、Issue 或聊天记录。
