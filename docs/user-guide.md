...
Completed scans are written to a private native directory index. The sunburst's **File paths** view follows the real hierarchy, while **Purpose** regroups common downloads, caches, and app data as a supporting view. Opening a folder, returning to its parent, or using breadcrumbs queries that index instead of reading the filesystem again. The first view contains the largest 24 items plus an “other items” summary; the side list can load more indexed items in pages. On exceptionally large trees, CoreRobin keeps the scanned totals accurate while folding lower-priority branches into “other items” so the local index cannot grow without bound. To inspect an aggregated branch in detail, start a folder scan closer to that location.
