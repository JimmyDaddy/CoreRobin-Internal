...
- Apps is a top-level entry in both Everyday and Professional mode. Its Application Center keeps current impact, local history, startup/network relationships, and uninstall management in one place. Select an app once to move between those views without having to identify it again on separate pages.
...
When no result exists yet, Cleanup first shows the scan scope, read-only boundary, and a **Start read-only scan** button; live progress appears only after scanning begins. For the system disk, choose **Quick scan** (recommended) or **Complete scan**. Quick scan precisely checks the locations listed before it starts—personal folders, Trash, application and developer caches, app-support/container data, hidden app data, and platform temporary folders—and clearly states that unlisted disk areas were not scanned. Complete scan traverses the whole selected filesystem. External/removable volumes and folders chosen through the system picker always use Complete scan because they have no trustworthy common-location definition. The five most recent non-system targets stay locally for quick reuse. Every scan remains inside the selected filesystem boundary and does not cross into another mount.

Completed scans are written to a private, layered native directory index. The sunburst's **File paths** view follows the real hierarchy, while **Purpose** regroups common downloads, caches, and app data as a supporting view. Every readable folder in the selected scope is retained in the index, including deep macOS package and system-directory contents. Opening a folder, returning to its parent, using breadcrumbs, or loading another page queries only that index and never reads the filesystem again. The first view contains the largest 24 items, with remaining indexed directories available through pagination. Each directory keeps up to 64 of its largest individual file details; additional files are combined into a terminal **Other content** total that never hides child folders or triggers another scan when selected.
...
One detailed index per scan target stays on this computer for up to 7 days and is marked older after 24 hours. If a viewed directory changes after the scan, CoreRobin keeps the existing map usable and shows a **Refresh this folder** action; it never starts a hidden recursive refresh when the window regains focus. An explicit refresh runs as a cancellable background job, keeps the old index visible, reconnects after the interface reloads, and replaces only that directory after a successful transaction. Cleanup uses indexed scan evidence to open each confirmed target through a stable filesystem boundary. Existing regular files and folders are removed, missing items are treated as already gone, and inaccessible or unsupported objects are skipped and reported. Opening review or switching cleanup modes does not rescan.
...
- Settings groups interface, background and desktop, sampling and views, notifications, and **Data & privacy** into consistent card grids and controls. The privacy center shows item count, size, update time, retention, and the native space-scan index footprint for resource and application-impact history, connection/network-quality history, application inventory, and scan results. Categories can be cleared separately with success/failure receipts; clear-all uses the same confirmation and retryable partial-failure flow.
...
- The task-center button in the top bar follows long-running cleanup, folder refresh, file checks, startup inventory, and update tasks across pages. It shows progress, failures, cancel/retry actions, and a direct return to the page that owns each task.
