...
Completed scans are written to a private, layered native directory index. The sunburst's **File paths** view follows the real hierarchy, while **Purpose** regroups common downloads, caches, and app data as a supporting view. Opening a cached folder, returning to its parent, or using breadcrumbs queries that index instead of reading the filesystem again. The first view contains the largest 24 items plus a paged summary. A complete scan preserves sibling directories and actionable home-folder levels first; deep macOS package contents, system internals, and branches outside the current working set appear as **Details not loaded**. Selecting that item loads only the current folder in a cancellable background task. A successful transaction replaces that index segment and may fold unrelated cached details back into loadable summaries, avoiding both a whole-disk rescan and unbounded index growth.
