...
- **Space-scan results**: normal scans read metadata and retain one detailed native SQLite index per scan target for up to seven days. Quick scan precisely walks only the common locations listed before it starts; Complete scan walks the whole selected filesystem boundary. The browsable index is bounded to 250,000 nodes and has a 1 GiB hard database limit. It keeps the largest directly useful entries in each folder, folds the rest into truthful “other items” totals, and stores descendant paths as a compact parent/name hierarchy instead of repeating the full path in every row. The index stores names, reconstructed paths, sizes, item counts, modification times, filesystem identities, and safety classifications—not file contents, hashes, or command lines—and is created with current-user-only permissions. Opening indexed folders does not read the filesystem again. Explicit directory refreshes keep the previous index usable until a replacement is committed successfully. Up to three compact growth snapshots per scan target retain only target identity, aggregate size, and top-level path/size summaries. The duplicate-file check runs only when explicitly started and reads candidate contents to hash them. Its result is retained for up to seven days with paths, sizes, modification time, device ID, inode, and local hashes; file contents are not stored. Cleanup uses indexed scan evidence to bind each confirmed target without following symbolic links; missing targets are skipped and inaccessible or unsupported targets are reported rather than blocking unrelated items.
