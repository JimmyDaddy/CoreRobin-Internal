...
- **Space-scan results**: normal scans read metadata and retain one layered native SQLite index per scan target for up to seven days. Quick scan precisely walks only the common locations listed before it starts; Complete scan walks the whole selected filesystem boundary. Each browsable working set is bounded to 250,000 nodes and the database has a 512 MiB hard limit. The index preserves sibling directories and actionable home-folder levels first, while macOS packages, deep system content, and branches outside the current working set remain accurate, loadable summaries. Loading one directory may fold unrelated cached details back into summaries instead of appending without limit. Descendant paths use a compact parent/name hierarchy rather than repeating the full path in every row. The index stores names, reconstructed paths, sizes, item counts, modification times, filesystem identities, and safety classifications—not file contents, hashes, or command lines—and is created with current-user-only permissions. Opening cached folders does not read the filesystem again. Explicit loads and refreshes keep the previous index usable until a replacement is committed successfully. Up to three compact growth snapshots per scan target retain only target identity, aggregate size, and top-level path/size summaries. The duplicate-file check runs only when explicitly started and reads candidate contents to hash them. Its result is retained for up to seven days with paths, sizes, modification time, device ID, inode, and local hashes; file contents are not stored. Cleanup uses indexed scan evidence to bind each confirmed target without following symbolic links; missing targets are skipped and inaccessible or unsupported targets are reported rather than blocking unrelated items.
