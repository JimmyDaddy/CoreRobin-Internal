...
- **Space-scan results**: normal scans read metadata and retain one layered native SQLite index per scan target for up to seven days. Quick scan precisely walks only the common locations listed before it starts; Complete scan walks the whole selected filesystem boundary. Every readable directory in that scope is retained regardless of the file-detail budget. The index keeps up to 250,000 individual file rows; additional files remain represented by directory totals. Near 512 MiB, CoreRobin retires surplus generations and transient recovery identities first instead of truncating the current directory tree to meet a fixed size. Descendant paths use a compact parent/name hierarchy rather than repeating the full path in every row. The index stores names, reconstructed paths, sizes, item counts, modification times, filesystem identities, and safety classifications—not file contents, hashes, or command lines—and is created with current-user-only permissions. Folder navigation and pagination query SQLite without reading the filesystem again. Only an explicit refresh re-reads a changed subtree; the previous index stays usable until the replacement commits, and unrelated directory details are retained. Up to three compact growth snapshots per scan target retain only target identity, aggregate size, and top-level path/size summaries. The duplicate-file check runs only when explicitly started and reads candidate contents to hash them. Its result is retained for up to seven days with paths, sizes, modification time, device ID, inode, and local hashes; file contents are not stored. Cleanup uses indexed scan evidence to bind each confirmed target without following symbolic links; missing targets are skipped and inaccessible or unsupported targets are reported rather than blocking unrelated items.
...
| --- | --- | --- |
| macOS | `~/Library/Application Support/com.corerobin.monitor/` (including bounded native history segments and the `cleanup-scan-index-v1.sqlite` space-scan index; temporary WAL and parallel-scan files are cleared with it); possible WebView or system-preference leftovers include `~/Library/WebKit/com.corerobin.monitor/`, `~/Library/Caches/com.corerobin.monitor/`, and `~/Library/Preferences/com.corerobin.monitor.plist` | `~/Library/LaunchAgents/core-robin.plist` |
