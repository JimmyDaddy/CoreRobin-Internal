...
最后核对：CoreRobin 0.1.2（2026-07-18）
...
- CoreRobin 0.1.2 不包含产品遥测、崩溃上报 SDK 或账户同步服务。
...
- 网络质量检查只在你点击开始后执行：通过系统 DNS 解析 `example.com`，并向其 443 端口建立最多 6 次 TCP 连接来估算延迟、抖动和丢包；不发送 HTTP 请求内容。
